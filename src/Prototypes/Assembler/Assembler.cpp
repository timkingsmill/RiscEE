...
void expand_text_section(const std::vector<std::string>& text_section, 
                         const std::vector<std::string>& instruction_formats,
                               std::vector<std::string>& instructions)
{
    for (std::string line : text_section)
    {
        ...
        while (line_stream >> token)
        {
            ...
            {
                // The current token should be an instruction mnemonic.
                std::string instruction_format = get_instruction_format(token, instruction_formats);
                if (!instruction_format.empty())
                {
                    // The mnemonic is valid because the instruction format is known.
                    std::cout << instruction_format << std::endl;
                }

                std::stringstream instruction_stream;
                instruction_stream << "    " << token;
                while (line_stream >> token)
                {
                    instruction_stream << ' ' << token;
                }
                instructions.push_back(instruction_stream.str());
            }
        }
    }
}
